...
public abstract class NeedBase
{
    ...
    /// <summary>Valeur maximale du besoin (satisfaction complète)</summary>
    private float _maxValue;

    /// <summary>Vitesse de diminution du besoin par seconde</summary>
    private float _decreaseRate;

    /// <summary>Urgence calculée du besoin (0 à 1)</summary>
    ...
    /// Automatiquement clampée aux bornes valides.
    /// </summary>
    public float NeedValue
    {
        ...
        private set
        {
            // Clampé entre 0 et MaxValue pour éviter les valeurs invalides
            _needValue = Mathf.Clamp(value, 0, MaxValue);
        }
    }

    /// <summary>
    /// Valeur maximale du besoin (état de satisfaction complète).
    /// Minimum de 1f pour éviter les divisions par zéro.
    /// Défini lors de l'initialisation via NeedConfig.
    /// </summary>
    public float MaxValue
    {
        ...
        private set
        {
            // Minimum 1 pour éviter les divisions par zéro dans Priority
    ...
    /// Minimum 0f pour éviter les vitesses négatives.
    /// </summary>
    public float DecreaseRate
    {
        ...
        private set
        {
            // Minimum 0 pour éviter une augmentation involontaire
    ...
    public float CriticalThreshold
    {
        ...
        private set
        {
            // Clampé entre 0 et 100 (pourcentage)
    ...
    /// Déclenche les comportements prioritaires du chien (manger, dormir, etc.).
    /// </summary>
    public bool IsCritical => NeedValue <= CriticalThreshold;

    /// <summary>
    /// Initialise le besoin avec sa configuration.
    /// Récupère le nom, la valeur max, le seuil critique et la vitesse de diminution.
    /// </summary>
    /// <param name="config">Asset NeedConfig contenant les paramètres du besoin</param>
    public NeedBase(NeedConfig config)
    {
        // Récupérer le nom depuis la config (ou "Need" par défaut)
        Name = config.needName is not null ? config.needName : "Need";
        
        // Initialiser les valeurs (les propriétés appliquent les clamps automatiquement)
        MaxValue = config.maxValue;
        NeedValue = MaxValue; // Commencer complètement satisfait
        CriticalThreshold = config.criticalThreshold;
        DecreaseRate = config.DecreaseRate;
    }

    /// <summary>
    /// Augmente la satisfaction du besoin (ex : après un repas).
    /// Ajoute de la valeur au NeedValue (automatiquement limité à MaxValue).
    /// Peut être surchargée dans les classes dérivées pour des comportements spécifiques.
    /// </summary>
    ...
    {
        // Augmenter la valeur du besoin (clampée automatiquement par la propriété)
        NeedValue += amount;
    }
    ...
    /// Peut être surchargée dans les classes dérivées pour ajouter des logiques spécifiques.
    /// </summary>
    public virtual void Process()
    {
        // Diminuer la valeur du besoin proportionnellement au temps écoulé
